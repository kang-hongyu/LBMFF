...
class NerProcessor(DataProcessor):
    ...
    def get_test_examples(self,data_dir,filename):
        return self._create_example(
            self._read_data(os.path.join(data_dir, filename)), "test"
        )
    ...
    def get_labels(self):
        ...
        return ["[PAD]","O","drug_B","gene_B","mutation_B","disease_B","body_part_B","daily_dose_B","dose_adjustment_B","dose_form_B","frequency_B","take_with_food_or_not_B","age_B","gender_B","drug_M","gene_M","mutation_M","disease_M","body_part_M","daily_dose_M","dose_adjustment_M","dose_form_M","frequency_M","take_with_food_or_not_M","age_M","gender_M","drug_E","gene_E","mutation_E","disease_E","body_part_E","daily_dose_E","dose_adjustment_E","dose_form_E","frequency_E","take_with_food_or_not_E","age_E","gender_E","drug_S","gene_S","mutation_S","disease_S","body_part_S","daily_dose_S","dose_adjustment_S","dose_form_S","frequency_S","take_with_food_or_not_S","age_S","gender_S","X","[CLS]","[SEP]"]
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    :param example:
    :param label_list: all labels :param max_seq_length:
    ...
    for i,label in enumerate(label_list):
        label_map[label] = i
    # with open("label2id1.pkl",'wb') as w:
...
def create_model(bert_config, is_training, input_ids, mask,
    ...
    lstm = True
    if lstm:
        bsz = tf.shape(output_layer)[0]
        cell_fw = tf.nn.rnn_cell.LSTMCell(256)
        cell_bw = tf.nn.rnn_cell.LSTMCell(256)
        output_layer, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, output_layer, dtype=tf.float32)
        output_layer = tf.concat(output_layer, axis=2)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        id2label = {key: value for key, value in enumerate(label_list)}
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, filename)
