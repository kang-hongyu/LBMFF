...
class GraphConvolutionDnn():
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            x = tf.matmul(x, self.vars['weights2'])
            x = tf.sparse_tensor_dense_matmul(self.adj, x)
            outputs = self.act(x)
        return outputs
...
class InnerProductDecoderDnn():
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            R = tf.matmul(R, self.vars['weights2'])
            D = tf.transpose(D)
            x = tf.matmul(R, D)
            x = tf.reshape(x, [-1])
            outputs = self.act(x)
